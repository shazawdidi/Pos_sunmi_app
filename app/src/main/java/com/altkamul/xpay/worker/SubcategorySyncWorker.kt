...
@HiltWorker
public class SubcategorySyncWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        Timber.d("Getting the updated subcategory using a worker :) ")
        ...
        val response = api.syncSubCategories(currentVersion = currentVersion)
        /** We had the response */
        Timber.d("We got a response !")
        when(response){
            is ServerResponse.Success -> {
                val subcategories = response.data
                ...
                dao.updateSubCategories(subCategories = subcategories)
                /** After this , we should update our last version of subcategory */
                dao.updateSubcategoryVersion(currentVersion,lastVersion)
                /** return a success response */
                return Result.success()
            }
            is ServerResponse.Error -> {
                /** Shit , we had an error but trying again */
                Timber.d("Server error when getting updated subcategories")
                return Result.retry()
            }
        }
    }
